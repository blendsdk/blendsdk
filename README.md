...
Every `blendsdk` install ships an [Agent Skill](https://agentskills.io/) for coding agents
...
Or install and update it with one command:

```bash
npx -y blendsdk@latest skill install
```

The installer finds the agent skill directories on your machine and asks which to install into.
...
`blendsdk skill status` shows the installed version and `blendsdk skill uninstall` removes it. Pin
a version with `npx -y blendsdk@5.55.0 skill install`.

Claude Code also discovers `.claude/skills/blendsdk`. See the
[docs](packages/blendsdk-docs/docs/getting-started/agent-skill.md) for the full list of
discovery paths.
