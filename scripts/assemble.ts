...
/** Source directory of the agent skill. */
const SKILL_SRC = path.join(ROOT, '.agents', 'skills', 'blendsdk');

/** Destination of the agent skill inside the published package. */
const SKILL_DEST = path.join(ROOT, 'packages', 'blendsdk', 'skills', 'blendsdk');

/** Source of the skill installer and the package bin dispatcher. */
const INSTALLER_SRC = path.join(ROOT, 'scripts', 'skill', 'install-skill.mjs');
const BIN_SRC = path.join(ROOT, 'scripts', 'skill', 'bin.mjs');

/** Destinations of the installer and bin dispatcher inside the package. */
const INSTALLER_DEST = path.join(OUTPUT_DIR, 'skill', 'install-skill.mjs');
const BIN_DEST = path.join(OUTPUT_DIR, 'bin', 'blendsdk.mjs');
...
const MODULES: ModuleMapping[] = [
  ...
  { name: 'authz', srcDir: 'packages/authz/dist', destDir: 'authz' },
  { name: 'webafx-authz', srcDir: 'packages/webafx-authz/dist', destDir: 'webafx-authz' },
  ...
  { name: 'api-client', srcDir: 'packages/api-client/dist', destDir: 'api-client' },
...
/**
 * Copies the agent skill into the published package.
 *
 * The destination is removed first so files deleted from the source cannot
 * linger in the packaged output.
 *
 * @param skillSrc - Absolute path to `.agents/skills/blendsdk`
 * @param skillDest - Absolute path to the packaged `skills/blendsdk`
 * @returns Number of files copied
 * @throws Error when the source skill tree does not exist
 */
export function assembleSkill(skillSrc: string, skillDest: string): number {
  if (!exists(skillSrc)) {
    throw new Error(`Skill source not found: ${skillSrc}`);
  }
  fs.rmSync(skillDest, { recursive: true, force: true });
  return copyDirRecursive(skillSrc, skillDest);
}
...
/**
 * Rewrites @blendsdk/* specifiers in a file so the assembled package never
 * names the private workspace scope.
 *
 ...
 *
 * Real (non-comment) imports become relative paths so the bundled code runs.
 * The relative path is computed from the file's location within the assembled
 * dist/ structure: a file at dist/dbcore/from-statement.js importing
 * @blendsdk/expression becomes ../expression/index.js.
 *
 * Specifiers on comment lines (JSDoc examples and quoted option defaults) are
 * consumer-facing documentation, so they become the public umbrella path
 * (`blendsdk/expression`) instead — a relative path would be meaningless once
 * published.
 *
...
export function rewriteImports(
   ...
   *
   * Captures:
   *   $1 = import keyword and any opening parenthesis (e.g. "from ", "require(")
   *   $2 = opening quote (' or ")
   *   $3 = package name (e.g., "expression", "webafx-cache")
   *   $4 = optional subpath (e.g., "/node" for @blendsdk/i18n/node)
   */
  const importPattern =
    /\b(from\s+|import\s*\(|import\s+|require\s*\()(['"])@blendsdk\/([a-z0-9-]+)(\/[a-z0-9-]+)?\2/g;
  ...
  const rewritten = content.replace(
    importPattern,
    (_match, prefix, q1, pkgName, subpath, offset: number) => {
      // A JSDoc code sample is consumer-facing documentation, so an import in a
      // comment points at the public umbrella path. A relative module path would
      // be meaningless once the package is published.
      if (isCommentLine(content, offset)) {
        rewrites++;
        return `${prefix}${q1}blendsdk/${pkgName}${subpath ?? ''}${q1}`;
      }
      ...
      return `${prefix}${q1}${relativePath}${q1}`;
    }
  );

  // A JSDoc sample can quote a package specifier outside an import statement,
  // for example an option default like `runtimeImport: '@blendsdk/api-client'`.
  // Those still name the private workspace scope, so publicize them as well.
  const publicized = rewritten.replace(/@blendsdk\//g, (match, offset: number) => {
    if (!isCommentLine(rewritten, offset)) {
      return match;
    }
    rewrites++;
    return 'blendsdk/';
  ...
  return { content: publicized, rewrites };
}

/**
 * Reports whether an offset falls on a comment line.
 *
 * TypeScript emits JSDoc with a leading `*`, so a documentation code sample is
 * recognized without a full JavaScript parser. A line comment (`//`) and the
 * first line of a block comment (`/*`) are treated the same way.
 *
 * @param content - The full file content.
 * @param offset - The offset of a regex match.
 * @returns True when the match starts on a comment line.
 */
function isCommentLine(content: string, offset: number): boolean {
...
/**
 * Scans assembled .js and .d.ts files for any quoted `@blendsdk/` specifier.
 *
 * This mirrors the release gate: the published package must never name the
 * private workspace scope, whether the specifier sits in real code or in a
 * JSDoc sample. Comments are checked too, so a future rewrite cannot silently
 * reintroduce the private scope into published documentation.
 *
 * @param dir - Directory to scan; defaults to the assembled output
 * @returns Array of file paths (relative to `dir`) with a remaining reference
 */
export function verifyNoRemainingReferences(dir: string = OUTPUT_DIR): string[] {
  const refPattern = /['"]@blendsdk\//;
  const violatingFiles: string[] = [];

  for (const filePath of findFiles(dir, REWRITABLE_EXTENSIONS)) {
    if (refPattern.test(fs.readFileSync(filePath, 'utf8'))) {
      violatingFiles.push(path.relative(dir, filePath));
    }
  }
...
function verifyBinContract(): string[] {
  ...
  if (target !== './dist/bin/blendsdk.mjs') {
    failures.push('the "blendsdk" executable must target ./dist/bin/blendsdk.mjs');
    return failures;
  }
  ...
  // The codegen CLI is spawned directly by workspace scripts (for example the
  // playground migration smoke test), so it must stay executable even though
  // the package bin now points at the dispatcher.
  const codegenCli = path.join(path.dirname(packagePath), 'dist', 'codegen', 'cli.js');
  if (exists(codegenCli)) {
    fs.chmodSync(codegenCli, fs.statSync(codegenCli).mode | 0o111);
  }
...
function assemble(): void {
  ...
  // Step 2b: Copy the agent skill into the published package
  console.log('📦 Copying agent skill...');
  const skillFiles = assembleSkill(SKILL_SRC, SKILL_DEST);
  console.log(`✔ Copied the agent skill (${skillFiles} files)\n`);

  // Step 2c: Copy the skill installer and the bin dispatcher
  console.log('📦 Copying the skill installer...');
  for (const [src, dest] of [
    [INSTALLER_SRC, INSTALLER_DEST],
    [BIN_SRC, BIN_DEST],
  ] as const) {
    if (!exists(src)) {
      throw new Error(`Installer source not found: ${src}`);
    }
    fs.mkdirSync(path.dirname(dest), { recursive: true });
    fs.copyFileSync(src, dest);
  }
  console.log('✔ Copied the skill installer and bin dispatcher\n');
...
// Run only when executed directly, so tests can import `assembleSkill` without
// assembling the real repository.
