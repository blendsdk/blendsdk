...
function publishAll({
  ...
  // Verify the agent skill is present so a release cannot publish without it
  const skillFile = path.join(blendPkgDir, 'skills', 'blendsdk', 'SKILL.md');
  if (!exists(skillFile)) {
    throw new Error('Agent skill missing from the package. Run `yarn build` first.');
  }
...
Notes:
  Publishes 'blendsdk' (umbrella SDK).
