...
/**
 * How a generated client should read a successful response body.
 *
 * - `'data'` (default): the response is `{ success: true, data }`, so the client
 *   returns the `data` value.
 * - `'body'`: the client returns the entire body, which is required for
 *   paginated responses that also carry a `pagination` object.
 */
export type ResponseEnvelope = 'data' | 'body';
...
export interface OpenAPIRouteMetadata {
  ...
  deprecated?: boolean;
  /**
   * How a generated client reads a successful response body.
   *
   * Defaults to `'data'`, which unwraps the top-level `data` property. A route
   * that returns a paginated body through `BaseController.paginated()` must set
   * `'body'`; otherwise the client returns only `data` and drops `pagination`.
   */
...
export interface RouteDefinition {
  ...
  validation?: ZodType;
  /**
   * Require authentication for this route, and optionally choose the service
   * container that holds the authenticated principal.
   *
   * - `true`: the guard resolves the default `'user'` service.
   * - a string: the guard resolves that service name instead. Use it when more
   *   than one auth provider runs in the same application; it must be the
   *   principal service name (the auth plugin's `userServiceName`), not the
   *   provider singleton.
   *
   * A route is secure when this is `true` or a string. Omitting `secure` (or
   * setting it to `false`) leaves the route public.
   */
...
export class RouteBuilder {
  ...
  /**
   * Marks the route as requiring authentication and optionally selects which
   * service container holds the authenticated principal.
   *
   * The authenticated principal must be present in the request services under
   * the chosen name; otherwise the request is rejected with a 401.
   *
   * @param userServiceName - Name of the principal service to resolve. Omit to
   *   use the default `'user'` service. Use a custom name when more than one
   *   auth provider runs in the same application; it must be the principal
   *   service name (the auth plugin's `userServiceName`), not the provider
   *   singleton. Must not be blank; surrounding whitespace is trimmed.
   * @returns This builder instance for method chaining
   * @throws {Error} If a blank or empty service name is provided
   *
   * @example
   * ```typescript
   * // Default 'user' principal
   * this.route().get('/me').secure().handle(this.me);
   *
   * // A separate principal service for machine clients
   * this.route().get('/export').secure('client').handle(this.export);
   * ```
   */
  secure(userServiceName?: string): this {
    if (userServiceName === undefined) {
      this.definition.secure = true;
    } else {
      // Trim first so a stray " client " resolves the same service as "client";
      // a value that is empty after trimming is a mistake, not a route name.
      const name = userServiceName.trim();
      if (name.length === 0) {
        throw new Error('secure() requires a non-empty user service name');
      }
      this.definition.secure = name;
    }
    return this;
  }
