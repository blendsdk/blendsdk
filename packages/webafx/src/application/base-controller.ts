...
export abstract class BaseController {
  ...
  /**
   * Creates a new route builder with authentication required, and optionally
   * selects which service container holds the authenticated principal.
   *
   * Shorthand for `route().secure(userServiceName)`.
   *
   * @param userServiceName - Service name to resolve. Omit to use the default
   *   `'user'` service; pass a custom name to authenticate against a different
   *   principal service.
   * @returns A new RouteBuilder instance with the secure flag set
   *
   * @example
   * ```typescript
   * // Default 'user' principal
   * this.authenticated().get('/me').handle(this.me);
   *
   * // A separate principal service for machine clients
   * this.authenticated('client').get('/export').handle(this.export);
   * ```
   */
  protected authenticated(userServiceName?: string): RouteBuilder {
    return this.route().secure(userServiceName);
  }
