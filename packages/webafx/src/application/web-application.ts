...
/**
 * Default service container name that holds the authenticated principal.
 * Routes that name no principal service resolve this one.
 */
const DEFAULT_PRINCIPAL_SERVICE = 'user';

/**
 * Whether a route requires authentication.
 *
 * A route is secure when `secure` is `true` or names a principal service. Any
 * string counts, so a blank name cannot silently make a route public — the
 * lookup fails and the request is rejected as unauthenticated.
 *
 * @param route - The route definition to inspect
 * @returns True when the route requires an authenticated principal
 */
function isSecureRoute(route: RouteDefinition): boolean {
  return route.secure === true || typeof route.secure === 'string';
}

/**
 * Resolves the service container name that holds the principal for a route.
 *
 * @param route - The route definition to inspect
 * @returns The name from `secure` when it is a string, otherwise the default
 *   `'user'` service
 */
function principalServiceName(route: RouteDefinition): string {
  return typeof route.secure === 'string' ? route.secure : DEFAULT_PRINCIPAL_SERVICE;
}
...
export class WebApplication {
  ...
  constructor(config?: ApplicationConfig) {
    ...
    // Null-prototyped maps so a service named "__proto__" is stored safely.
  ...
  protected wrapRouteHandler(controller: unknown, route: RouteDefinition): RequestHandler {
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        // 1. Resolve the principal once when the route needs it: either to
        //    authenticate the request or to pass to the authorization callback.
        //    The name comes from the route (default 'user'), so one application
        //    can keep more than one principal service apart.
        const secured = isSecureRoute(route);
        const principal: unknown =
          secured || route.authorize
            ? await req.services.get(principalServiceName(route), undefined)
            : undefined;

        // 2. Reject an unauthenticated request on a secure route. An
        //    unregistered or empty service name fails closed here.
        if (secured && !principal) {
          throw new UnauthorizedError('Authentication required');
        }

        // 3. Authorize against the same principal the route selected.
        if (route.authorize) {
          const authorized = await route.authorize(req, principal);
          if (!authorized) {
