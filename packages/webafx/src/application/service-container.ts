...
export class ServiceContainer {
  ...
  constructor(
    ...
    this.response = res;
    this.next = next;
    // Pre-populate with existing singletons from registry. Null-prototyped so
    // a service named "__proto__" or "constructor" is not shadowed by an
    // inherited property.
    this.services = Object.assign(Object.create(null) as Record<string, unknown>, registry.singletons);
   ...
   *
   * A duplicate name is rejected rather than silently overwriting the earlier
   * definition. To replace a resolved instance for one container, use
   * {@link set} instead.
   *
  ...
  registerService(service: ServiceDefinition): void {
    if (Object.prototype.hasOwnProperty.call(this.registry.definitions, service.name)) {
