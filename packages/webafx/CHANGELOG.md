...
Added:
- Document the OpenAPI response envelope and refresh skill freshness, including `ResponseEnvelope` and the `.openapi()` `envelope` option.

Changed:
- Set `envelope: 'body'` on paginated examples to maintain `pagination`.
- Regenerate codegen and webafx packages, refreshing the skill and updating manifest hashes for versionless sources.
- Drop the version metadata from ai-training templates and sanitize injected package.json context.

Fixed:
- Ensure plugin and service registries have a null prototype to reject duplicate names.
- Correct the secure() JSDoc to clarify service name handling, allowing explicit 'user' service resolution.

Security:
- Enhance secure routes to select their principal service, expanding RouteDefinition.secure to accept both boolean and string values.


## [5.56.0] - 2026-09-15

- Added: Duplicate registration tests for plugins and services to ensure proper behavior on name conflicts.
- Added: Implementation tests for secure storage and error handling for blank routes.
- Added: Principal service selection for secure routes, allowing explicit service names.
- Changed: Regenerated codegen and webafx packages with updated skill references and manifest hashes.
- Changed: Existing ai-training templates to remove version metadata for consistency.
- Fixed: Plugin and service registries to prevent overwriting on duplicate registrations by using null prototypes.
- Fixed: Secure route documentation to clarify service name requirements and functionality.


## [5.55.0] - 2026-09-14

## Added
- Assembled agent skill from 176 regenerated AI training sources using DeepSeek. 

## Changed
- Migrated retired MCP hand-written guides and patterns to updated documentation format.

## Fixed
- Updated various documentation files for clarity and accuracy across all training materials.
