...
describe('BlendScript built distribution', () => {
  ...
  it('should keep package references and the standalone course discoverable', () => {
    ...
    ).toContain('blendsdk/blendscript');
    expect(
      readFileSync(
        path.join(
          repositoryRoot,
  ...
  it('should pack only the approved public BlendScript artifacts', () => {
    ...
    expect(paths).toContain('skills/blendsdk/SKILL.md');
    const blendScriptPaths = paths.filter(filePath => filePath.startsWith('dist/blendscript/'));
    expect(blendScriptPaths.length).toBeGreaterThan(0);
    expect(
