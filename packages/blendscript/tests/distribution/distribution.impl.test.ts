...
describe('BlendScript distribution implementation', () => {
  it('should use exact direct and umbrella manifest ownership', () => {
    ...
    expect(umbrella.files).toEqual(['dist', 'README.md', 'skills']);
  });
  ...
  it('should generate documentation only through the existing source owners', () => {
    ...
    const overviewSentence = 'expression language';
    const techdocsSentence = 'deterministic expression language for business rules';
    ...
    const skillPackageDocumentation = readFileSync(
      path.join(
        repositoryRoot,
        '.agents',
        'skills',
    ...
    expect(overview).toContain(overviewSentence);
    expect(packageDocumentation).toContain(techdocsSentence);
    expect(packageDocumentation).not.toContain(courseTitle);
    expect(skillPackageDocumentation).toContain(overviewSentence);
    expect(skillPackageDocumentation).not.toContain(courseTitle);
    expect(
