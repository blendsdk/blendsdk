...
export interface AuthPluginOptions {
     ...
     * is authenticated.
     *
     * This is the counterpart of a route's principal selection. An unnamed
     * secure route always resolves the default `'user'` service, whatever this
     * option is; to authenticate against this name, name it on the route — for
     * example `secure('client')` on a route and `userServiceName: 'client'`
     * here.
     *
     * Default: `'user'`
     */
...
/**
 * Convenience factory: creates an OAuth2 token introspection auth plugin.
 *
 * Shorthand for `createAuthPlugin(new IntrospectionAuthProvider(config), options)`.
 * Accepts either static client credentials or a `configFactory` for DB-backed,
 * per-tenant credentials.
 *
 * @param config - Introspection configuration (endpoint, client credentials or configFactory)
 * @param options - Optional plugin options (serviceName, userServiceName, priority)
 * @returns PluginDefinition for use with `app.use()`
 *
 * @example
 ...
 *         const creds = await credentialsRepo.get(resolveTenant(req));
 *         return {
 *             introspectionUrl: creds.endpoint,
 ...
 * ```
 */
export function introspectionAuthPlugin(
    config: IntrospectionProviderConfig,
    options?: AuthPluginOptions,
): PluginDefinition {
    return createAuthPlugin(new IntrospectionAuthProvider(config), options);
}
