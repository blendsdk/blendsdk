...
export class JwtAuthProvider extends AuthProvider {
    ...
    async validate(token: string): Promise<AuthResult | undefined> {
        try {
            ...
            // token from another API is not accepted unchecked.
            if (this.config.requireAudience && !this.config.audience) {
                return undefined;
            }
            ...
            // Map the verified JWT payload to our standardized AuthResult
            // format, then stamp the configured principal type.
