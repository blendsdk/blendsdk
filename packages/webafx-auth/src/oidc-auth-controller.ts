 ...
 * public and validate the session cookie in the handler.
     ...
     * 10. Redirects to returnTo or '/' (P4)
     *
     ...
     * 4. Stores the updated session. When the provider enables
     *    `rotateSessionIdOnRefresh`, the session moves to a new opaque id, the
     *    old session is deleted, and the cookie is re-issued with the new id;
     *    otherwise the session is replaced under the existing id.
     * 5. Re-issues the session cookie with the provider session-cookie TTL so
     *    the cookie slides with the session (only after a successful store)
     * 6. Returns success with new expiry
     */
    async handleRefresh(req: Request, res: Response): Promise<void> {
        ...
        if (provider.shouldRotateSessionIdOnRefresh()) {
            // Rotate: store the updated session under a fresh id, remove the old
            // session, then hand the browser the new id. The cookie is set last,
            // so a failure before this point leaves the existing cookie (and its
            // session) in place.
            const newSessionId = randomUUID();
            await provider.storeSession(newSessionId, updatedSession);
            await provider.clearSession(sessionId);
            this.setCookie(res, cookieName, newSessionId, {
                maxAge: provider.getSessionCookieTtl(),
            });
        } else {
            ...
            // Re-issue the session cookie so the browser cookie and the
            // server-side session slide together. Only reached after a
            // successful store, so a failed refresh never extends the cookie.
            this.setCookie(res, cookieName, sessionId, {
                maxAge: provider.getSessionCookieTtl(),
            });
        }
