...
export abstract class AuthProvider {
    ...
    /**
     * Configured default principal type for results from this provider, if any.
     * Applied by {@link withPrincipalType} and the default claims mapper.
     */
    ...
    // -----------------------------------------------------------------------
    // Token Extraction (public — used by tenant-delegating providers and tests)
    // -----------------------------------------------------------------------
    ...
    protected defaultClaimsMapper(
        ...
        const result: AuthResult = { sub, claims: rawClaims, token, exp, scopes };
        if (this._principalType !== undefined) {
            result.principalType = this._principalType;
        }
        return result;
    }

    /**
     * Fills the configured principal type on a mapped result when the result
     * does not already carry one, so a custom claims mapper stays authoritative.
     *
     * @param result - The mapped authentication result, or undefined when the
     *   provider did not authenticate the token
     * @returns The result with `principalType` filled when it was unset, or
     *   undefined unchanged
     */
    protected withPrincipalType(result: AuthResult | undefined): AuthResult | undefined {
        if (result === undefined) {
            return undefined;
        }
        if (this._principalType === undefined || result.principalType !== undefined) {
            return result;
        }
        return { ...result, principalType: this._principalType };
    }
