...
// ---------------------------------------------------------------------------
// Specification Tests: required audience
// ---------------------------------------------------------------------------

describe("Specification: OidcAuthProvider required audience", () => {
    /** The static config without an audience, for fail-closed cases. */
    const noAudienceConfig: OidcAuthConfig = {
        serviceName: "oidc-test",
        issuerUrl: "https://auth.example.com",
        clientId: "test-client",
        clientSecret: "test-secret",
    ...
    it("should reject a token when the audience is required but not configured", async () => {
        const provider = new OidcAuthProvider({
            ...noAudienceConfig,
        ...
        expect(result).toBeUndefined();
        // Fail closed before verification — no needless verification work.
        expect(jose.jwtVerify).not.toHaveBeenCalled();
        // The gate runs before discovery, so no JWKS fetch happens either.
        expect(client.discovery).not.toHaveBeenCalled();
    });

    it("should reject a bearer request when the audience is required but not configured", async () => {
        const provider = new OidcAuthProvider({
            ...noAudienceConfig,
            requireAudience: true,
        });
    ...
    it("should honor a configFactory that requires an audience but returns none", async () => {
        const provider = new OidcAuthProvider({
            configFactory: async () => ({
                issuerUrl: "https://auth.example.com",
                clientId: "test-client",
                requireAudience: true,
                // no audience for this tenant
            }),
        });
    ...
    it("should skip the audience check when not required and no audience is configured", async () => {
        const provider = new OidcAuthProvider(noAudienceConfig);
    ...
    it("should pass the configured audience to verification when required", async () => {
        const provider = new OidcAuthProvider({
            ...baseConfig,
            requireAudience: true,
        });
    ...
    it("should pass an audience array to verification when required", async () => {
        const provider = new OidcAuthProvider({
            ...baseConfig,
        ...
        expect(jose.jwtVerify).toHaveBeenCalledWith(
            "some-token",
            expect.anything(),
            expect.objectContaining({ audience: ["api-a", "api-b"] })
        );
    });

    it("should return undefined when verification rejects a token whose audience is wrong", async () => {
    ...
    it("should return undefined for a bearer request whose audience is wrong", async () => {
        vi.mocked(jose.jwtVerify).mockRejectedValueOnce(
            new Error("unexpected aud claim value")
        );
        const provider = new OidcAuthProvider({
...
// ---------------------------------------------------------------------------
// Specification Tests: session expiry
// ---------------------------------------------------------------------------

describe("Specification: OidcAuthProvider session expiry", () => {
    /** Seconds since epoch, offset from now. */
    function epochFromNow(offsetSeconds: number): number {
        return Math.floor(Date.now() / 1000) + offsetSeconds;
    }

    /** Store a session and return a provider wired to the mock cache. */
    function providerWithSession(
        session: ReturnType<typeof createSampleSession>,
        config: Partial<OidcAuthConfig> = {},
    ): OidcAuthProvider {
        const { provider: cacheProvider, store } = createMockCacheProvider();
    ...
    it("should reject a session whose expiresAt is in the past", async () => {
        const provider = providerWithSession(
            createSampleSession({ expiresAt: epochFromNow(-3600) })
        );

        const result = await provider.authenticate(cookieRequest());

        expect(result).toBeUndefined();
    });

    it("should accept a session that expired within the configured clock tolerance", async () => {
        const provider = providerWithSession(
            createSampleSession({ expiresAt: epochFromNow(-10) }),
            { clockTolerance: 60 }
        );
    ...
    it("should reject a session just past expiry when clock tolerance is zero", async () => {
        const provider = providerWithSession(
            createSampleSession({ expiresAt: epochFromNow(-10) }),
            { clockTolerance: 0 }
        );
    ...
    it("should accept a session whose expiresAt is in the future", async () => {
        const provider = providerWithSession(
            createSampleSession({ expiresAt: epochFromNow(3600) })
        );
    ...
    it("should accept a session without expiresAt and rely on the store TTL", async () => {
        const provider = providerWithSession(
            createSampleSession({ expiresAt: undefined })
        );
