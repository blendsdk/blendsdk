...
describe("JwtAuthProvider", () => {
    ...
    describe("audience validation", () => {
        ...
        it("should reject every token when the audience is required but not configured", async () => {
            const requiredProvider = new JwtAuthProvider({
                secret: TEST_SECRET,
                algorithms: ["HS256"],
                issuer: TEST_ISSUER,
            ...
            const token = await signTestJwt({ audience: TEST_AUDIENCE });
            const result = await requiredProvider.validate(token);

            // Fail closed: with no audience configured, nothing is accepted.
            expect(result).toBeUndefined();
            await requiredProvider.shutdown();
        });

        it("should reject a wrong-audience token and accept the right one when required", async () => {
            const requiredProvider = new JwtAuthProvider({
                secret: TEST_SECRET,
                algorithms: ["HS256"],
                issuer: TEST_ISSUER,
            ...
            const wrong = await signTestJwt({ audience: "another-api" });
            expect(await requiredProvider.validate(wrong)).toBeUndefined();

            const right = await signTestJwt({ audience: TEST_AUDIENCE });
            expect(await requiredProvider.validate(right)).toBeDefined();
