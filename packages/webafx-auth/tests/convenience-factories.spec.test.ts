...
const INTROSPECTION_CONFIG: IntrospectionAuthConfig = {
...
describe("Convenience Factories — Specification Tests", () => {
    ...
    describe("ST-126: introspectionAuthPlugin()", () => {
        it("returns a PluginDefinition with name, priority, and factory", () => {
            const plugin = introspectionAuthPlugin(INTROSPECTION_CONFIG);
        ...
        it("forwards AuthPluginOptions to the plugin", () => {
            const plugin = introspectionAuthPlugin(INTROSPECTION_CONFIG, {
                serviceName: "introspection",
                priority: 7,
            });

            expect(plugin.name).toBe("auth:introspection");
