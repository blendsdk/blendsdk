...
// ---------------------------------------------------------------------------
// Session cookie TTL resolution internals
// ---------------------------------------------------------------------------

describe("Implementation: session cookie TTL resolution", () => {
    let cacheProvider: ReturnType<typeof createMockCacheProvider>["provider"];

    beforeEach(() => {
        cacheProvider = createMockCacheProvider().provider;
    });

    /** Build a provider with the given TTL settings. */
    ...
    // The cookie TTL must not change the server-side store TTL.
    it("should keep the store TTL independent of sessionCookieTtl", async () => {
        const customProvider = providerWith({ sessionTtl: 1800, sessionCookieTtl: 7200 });

        await customProvider.storeSession("uuid-1", SAMPLE_SESSION);
        ...
        expect(customProvider.getSessionCookieTtl()).toBe(7200);
    });

    // A deliberate 0 must not be treated as "unset" by the nullish chain.
    it("should treat an explicit sessionCookieTtl of 0 as configured", () => {
        const customProvider = providerWith({ sessionTtl: 1800, sessionCookieTtl: 0 });

        expect(customProvider.getSessionCookieTtl()).toBe(0);
    });
});
