...
describe("OidcAuthProvider — session data edge cases", () => {
    ...
    it("rejects a session whose expiresAt is in the past, even when the cache entry is still present", async () => {
        ...
        // Expiry is enforced in the provider, not left to the CacheProvider TTL.
        const result = await provider.authenticate(req);
        expect(result).toBeUndefined();
    });

    it("rejects a session whose expiresAt is exactly the clock-tolerance boundary", async () => {
        const nowSeconds = Math.floor(Date.now() / 1000);
        const boundarySession: OidcSession = {
            accessToken: "boundary-token",
            user: { sub: "boundary-user" },
            // The default tolerance is 30 seconds; at exactly now - 30 it is past.
            expiresAt: nowSeconds - 30,
        };
        cacheStore.set("oidc:session:boundary-sess", {
            value: boundarySession,
            expiresAt: 0,
        });

        const provider = createProviderWithSessionStore(cacheProvider);
        const req = createMockReq({
            headers: { cookie: "__oidc_session=boundary-sess" },
        });
    ...
    it("rejects a session whose expiresAt is the epoch (0)", async () => {
        const zeroSession: OidcSession = {
            accessToken: "zero-token",
            user: { sub: "zero-user" },
            expiresAt: 0,
        };
        cacheStore.set("oidc:session:zero-sess", { value: zeroSession, expiresAt: 0 });

        const provider = createProviderWithSessionStore(cacheProvider);
        const req = createMockReq({
            headers: { cookie: "__oidc_session=zero-sess" },
        });
