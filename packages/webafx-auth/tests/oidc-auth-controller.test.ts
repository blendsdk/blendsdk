...
describe("OidcAuthController", () => {
    ...
    describe("routes()", () => {
        ...
        it("marks me as authenticated (secure) and leaves logout/refresh self-validating", () => {
            const routes = controller.routes();
            // Logout and refresh validate the session cookie in the handler, so
            // an expired session can still refresh or log out.
            expect(routes[2].secure).toBeUndefined(); // logout
            expect(routes[3].secure).toBe(true); // me
            expect(routes[4].secure).toBeUndefined(); // refresh
        });
