...
class ImplTestController extends OidcAuthController {
    ...
    /**
     * Exposes the protected setCookie helper so a test can prove subclasses
     * reuse the framework's secure cookie options.
     */
    public setTestCookie(res: Response, name: string, value: string, maxAge: number): void {
...
// ---------------------------------------------------------------------------
// Sliding session cookie internals
// ---------------------------------------------------------------------------

describe("OidcAuthController — sliding session cookie internals", () => {
    it("subclasses can call the protected setCookie helper with secure defaults", () => {
        const controller = new ImplTestController(createMockSettings(), mockServices);
        const res = createMockRes();

        controller.setTestCookie(res as any, "__device", "fingerprint", 86400);

        expect(res._cookies["__device"]).toMatchObject({
            value: "fingerprint",
            options: { httpOnly: true, sameSite: "lax", path: "/", maxAge: 86_400_000 },
        });
    });

    it("re-issued session cookie keeps the secure attributes on refresh", async () => {
        const controller = new ImplTestController(createMockSettings(), mockServices);
        const routes = controller.routes();

        // Login + callback to create a stored session.
        const loginRes = createMockRes();
        await routes[0].handler(createMockReq(), loginRes as any, vi.fn());
        const stateVal = loginRes._cookies["__oidc_state"].value;
        ...
        const cbRes = createMockRes();
        await routes[1].handler(cbReq, cbRes as any, vi.fn());
        const sessionId = cbRes._cookies["__oidc_session"].value;
        ...
        expect(refreshRes._cookies["__oidc_session"]).toMatchObject({
            value: sessionId,
            options: { httpOnly: true, sameSite: "lax", path: "/", maxAge: 3_600_000 },
        });
    });
});
