...
// ---------------------------------------------------------------------------
// Session cookie TTL resolution
// ---------------------------------------------------------------------------

describe("Specification: session cookie TTL resolution", () => {
    /** Build a provider with only the fields this block exercises. */
    function providerWithTtl(config: Partial<OidcAuthConfig>): OidcAuthProvider {
    ...
    // The cookie lifetime can be set independently of the server session.
    it("should prefer sessionCookieTtl when both sessionCookieTtl and sessionTtl are set", () => {
        const provider = providerWithTtl({ sessionCookieTtl: 7200, sessionTtl: 1800 });

        expect(provider.getSessionCookieTtl()).toBe(7200);
    });

    // Unset cookie TTL means the cookie follows the server session.
    it("should fall back to sessionTtl when sessionCookieTtl is not set", () => {
        const provider = providerWithTtl({ sessionTtl: 1800 });

        expect(provider.getSessionCookieTtl()).toBe(1800);
    });

    // With nothing configured the cookie matches the 3600-second session default.
    it("should default to 3600 when neither sessionCookieTtl nor sessionTtl is set", () => {
        const provider = providerWithTtl({});

        expect(provider.getSessionCookieTtl()).toBe(3600);
    });
});
