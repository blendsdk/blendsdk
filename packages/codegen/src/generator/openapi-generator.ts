...
export class OpenAPIGenerator {
  ...
  generate(): OpenAPIDocument {
    ...
    // Move reusable Zod definitions into components.schemas and rewrite refs
    return hoistDefinitions(document);
  }
  ...
  protected buildPaths(): Record<string, OpenAPIPathItem> {
    ...
    // Sort both the path keys and each path item's method keys so the emitted
    // document is byte-stable regardless of controller registration order.
    const sortedPaths: Record<string, OpenAPIPathItem> = {};
    for (const key of Object.keys(paths).sort()) {
      sortedPaths[key] = sortByName(paths[key]);
    }
  ...
  protected buildOperation(definition: RouteDefinition): OpenAPIOperation {
    ...
    // Record how a generated client reads a successful response body. The
    // default 'data' unwraps the top-level data property; 'body' returns the
    // whole body, which paginated responses need to keep their pagination.
    operation['x-blend-envelope'] = metadata.envelope ?? 'data';
  ...
  protected buildParameters(definition: RouteDefinition): OpenAPIParameter[] {
    ...
    for (const paramName of pathParamNames) {
      ...
      if (paramMeta?.schema) {
        param.schema = this.convertZodToSchema(paramMeta.schema, 'input');
      } else {
  ...
  protected buildRequestBody(
    ...
    | { description?: string; required?: boolean; content: Record<string, { schema?: any }> }
    | undefined {
    ...
    const schema = this.convertZodToSchema(definition.validation, 'input');
  ...
  protected buildResponses(definition: RouteDefinition): Record<string, OpenAPIResponse> {
    ...
    for (const responseDef of metadata.responses) {
      ...
      // If a response schema is provided, convert it in the output direction so
      // defaults and transforms resolve to the shape the server returns.
      if (responseDef.schema) {
        response.content = {
          'application/json': {
            schema: this.convertZodToSchema(responseDef.schema, 'output'),
          },
        };
      }
   ...
   *
   * Delegates to the native `z.toJSONSchema` converter. The `direction` selects
   * which side of the wire the schema describes: `'input'` for schemas a caller
   * sends, `'output'` for schemas the server returns.
   *
   ...
   */
  protected convertZodToSchema(zodSchema: unknown, direction: 'input' | 'output'): OpenAPISchema {
    if (!isZodSchema(zodSchema)) {
      return { type: 'object' };
    }
    return convertZodToJsonSchema(zodSchema, direction);
  }
   ...
   *
   * Delegates to the splitter in the zod-to-openapi module. Optional and
   * defaulted properties become non-required parameters.
   *
  ...
  protected convertZodToQueryParameters(zodSchema: unknown): OpenAPIParameter[] {
    ...
    return convertZodToQueryParameters(zodSchema);
  }
}
