...
describe('OpenAPIGenerator — Security', () => {
  ...
  test('secure: a service name string still applies default security to operation', () => {
    const gen = createSecureGenerator();
    gen.addRoutes('/api', [
      createRoute({
        path: '/named-principal',
        secure: 'client',
        openapi: { summary: 'Endpoint for a named principal service' },
      }),
    ]);
    const doc = gen.generate();

    // A named principal service marks the route as secure, exactly like `true`.
    const operation = doc.paths['/api/named-principal'].get!;
    expect(operation.security).toEqual([{ bearerAuth: [] }]);
  });
