...
### Added
- Generate typed API clients and a `blendsdk api` CLI with operation grouping and naming fallback.
- Convert Zod schemas with native z.toJSONSchema and preserve envelope signal.
- Generate versionless ai-training and skill references by dropping version metadata lines.

### Changed
- Align initializer wording with the documentation contract by rewording initializer-DDL and testing guide.
- Refresh the codegen page and link the API client guide to document new features.

### Fixed
- Preserve JSDoc import examples during assembly by skipping comment lines when rewriting imports.
- Sanitize reserved component names to prevent drift with proper naming conventions.
- Ensure generated clients compile and fail checks on drift detection.
- Make schema hoisting collision-safe by renaming conflicting $defs deterministically.
- Conform generated examples to the public surface to ensure they only import public symbols.
- Keep the codegen CLI executable and update bin-contract tests to verify bin correctness.
...
### Security
- Ensure generated clients correctly validate and handle security schemes during API checking.


## [5.56.0] - 2026-09-15

### Changed
- Regenerated ai-training documentation and refreshed skill references to stay current.

### Fixed
- Kept the codegen CLI executable after updating bin-contract tests to ensure proper functionality.
- Conformed generated examples to the public API surface by regenerating multiple packages to fix import statements.

### Added
- Generated versionless ai-training and skill references by removing version metadata from templates.
- Assembled agent skill from regenerated sources, incorporating extensive documentation updates and template improvements.
...
Changed:
  - Regenerated codegen examples to ensure only public symbols are imported.
  - Updated documentation for various AI training concepts and best practices.

Removed:
  - Migrated retired MCP hand-written guides, patterns, and templates to streamline references.

Fixed:
  - Ensured that generated examples conform to the public surface of the API.
