...
- `useAuthorization` hook and the `RequireAccess` and `Can` components for role- and permission-based UI and routing.
- `notAuthorizedPath` auth configuration for the denied-access redirect.
...
Changed:
- Conform generated examples to the public surface by regenerating codegen, dbcore, expression, react, and webafx-cache to import only public symbols.
- Assemble agent skill from regenerated sources, migrating retired MCP guides and patterns.

Fixed:
- Sanitize the injected package.json context and remove reference header injection.


## [5.56.0] - 2026-09-15

- Added versionless AI-training and skill references to templates and removed version metadata.
- Changed all generated examples to conform to the public surface and regenerated relevant files.
- Assembled agent skill from regenerated sources and migrated retired MCP guides, patterns, and templates.


## [5.55.0] - 2026-09-14

Added:
- Assembled agent skill from regenerated sources with DeepSeek to enhance functionality.

Changed:
- Regenerated examples to conform to the public surface, ensuring they import only public symbols.
- Updated all related documentation files to reflect changes and enhancements in the package.

Removed:
- Migrated retired MCP hand-written guides and patterns to new structured formats and locations.
