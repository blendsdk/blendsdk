...
This package ships an agent skill under `skills/blendsdk` that teaches coding agents how to use
BlendSDK. Install or update it into your agents with one command:

```bash
npx -y blendsdk@latest skill install
```

The installer scans the agent skill directories on your machine and asks which to install into. For
non-interactive use:
...
npx -y blendsdk@latest skill install --project             # the current project
```
...
`blendsdk skill status` shows the installed version and `blendsdk skill uninstall` removes it. Pin a
version with `npx -y blendsdk@5.55.0 skill install`.

Prefer to do it by hand? Copy or symlink the shipped folder:

```bash
mkdir -p .agents/skills
cp -R node_modules/blendsdk/skills/blendsdk .agents/skills/blendsdk
# or, to track the installed package:
ln -s ../../node_modules/blendsdk/skills/blendsdk .agents/skills/blendsdk
```

Claude Code discovers skills in `.claude/skills/blendsdk`. Existing `blendsdk-mcp` installs keep
working until you remove them; the skill replaces the MCP documentation server.
...
| webafx-auth | `blendsdk/webafx-auth` | Token validation plugin (JWT/OIDC/introspection) | `jose` |
...
# Authentication — JWT/OIDC/introspection (webafx-auth)
npm install jose
